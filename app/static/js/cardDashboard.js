...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    submitButton.addEventListener('click', async () => {
        try {
            ...
            // 使用 fetch 將數據發送到後端
            const response = await fetch('/get_merch_by_id_user_card_ver', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    allButton.addEventListener('click', async () => {
        try {
            ...
            // 發送 GET 請求到後端
            const response = await fetch(`/get_user_cards_from_merch`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                },
    ...
    async function addPhotoCard(item) {
        ...
        try {
            const favResponse = await fetch(`/get_user_card_by_id?merch_id=${merch_id}`, {
                method: 'GET',
                headers: {
                    "Content-Type": "application/json",
                },
        ...
        const BtnTxt = is_card ? `<button class="delete-button" data-name="${name}" data-merch-id="${merch_id}" style="color: white; background-color: red; border: none; border-radius: 4px; padding: 5px; cursor: pointer;">-</button>` : `<button class="add-button" data-name="${name}" data-merch-id="${merch_id}" style="color: white; background-color: green; border: none; border-radius: 4px; padding: 5px; cursor: pointer;">+</button>`;
...
async function handleAdd(button) {
    ...
    try {
        const response = await fetch('/create_user_card', {
            method: 'POST',
...
async function handleDelete(button) {
    ...
    try {
        const response = await fetch('/delete_user_card', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ merch_id: merchId }),
        });
